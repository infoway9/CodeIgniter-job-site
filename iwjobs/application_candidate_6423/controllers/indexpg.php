...
class Indexpg extends CI_Controller {
    ...
    public function index() {
        ...
        //----------- start initializing valiable --------------//
        
        //---------- start populating companies ----------------//
        $company_arr=$this->Indexdb->all_company_logo()->result_array();
        
        $data_msg['companies']=$company_arr;
        //---------- end populating companies ----------------//
        
        
        //----------- end initializing variable ----------------//
        ...
        if ($this->input->post('btn_signin')) {
            ...
            if ($error_msg == "") {
                $id_qry = $this->Userdb->get_user_id($username);
                ...
                if ($id_qry->num_rows != 0) {
                    ...
                    if ($user_status == '0') {
                        ...
                        if ($username == "") {
                            ...
                            $get_userinformation = $this->Userdb->matchuser($username, $password);
                            ...
                            if ($get_userinformation->num_rows == 0) {
                                ...
                                if ($stay_signin == '1') {
                                    $cookie = array(
                                        'name' => 'job_autologin',
                                        'value' => $this->encrypt->encode($user_name . '~####~' . $password),
