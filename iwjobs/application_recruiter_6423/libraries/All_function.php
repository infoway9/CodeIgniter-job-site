...
class All_function {
    ...
    function get_recruiter_emaildetail($recruiter_id) {
        $CI = & get_instance();
        $CI->load->model('Profiledb', '', TRUE);

        $val = $CI->Profiledb->get_recruiter_details($recruiter_id)->result_array();

        $fullname = $val[0]['UserName'];
        $email = $val[0]['Email'];
        $organization=$val[0]['Organization'];
        $val = array('fullname' => $fullname, 'email' => $email,'organization'=>$organization);
    ...
    function get_user_emaildetail($user_id) {
        ...
        $val = $CI->All_functiondb->get_user_details($user_id)->result_array();
